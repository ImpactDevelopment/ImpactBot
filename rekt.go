...
func muteHandler(caller *discordgo.Member, msg *discordgo.Message, args []string) error {
	...
	if strings.ToLower(args[0]) == "tempmute" {
		...
		if err == nil {
			...
			// Insert new entry
			_, err = DB.Exec("INSERT INTO tempmutes (discord_id, channel_id, expiration) VALUES (?, ?, ?)", userId, channelId, expiration)
			if err != nil {
				return err
			}
		} else {
...
func init() {
	...
	go func() {
		...
		for range ticker.C {
			...
			now := time.Now().Unix()
			rows, err := DB.Query("SELECT id, discord_id, channel_id FROM tempmutes WHERE expiration < ?", now)
			if err != nil {
			...
			for rows.Next() {
				...
				_, err = DB.Exec("DELETE FROM tempmutes WHERE id = ?", id)
				if err != nil {
				...
				// Do the unmute
				fmt.Printf("Processing temp unmute for \"%s\" from channel \"%s\"\n", discordId, channelId.String)
