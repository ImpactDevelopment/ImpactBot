...
var replies = []Reply{
	...
	{
		pattern: `(?i)defender|virus|mcafee|norton|trojan|\srat`,
	...
	{
		pattern: `(?i)use\sbaritone|baritone\susage|baritone\scommand|[^u]\.b|goal|goto|path`,
		message: "Please read the [Baritone usage guide](https://github.com/cabaletta/baritone/blob/master/USAGE.md)",
	},
	{
		pattern:      `(?i)installe?r?|mediafire|dire(c|k)+to?\s+(linko?|url|site|page)|ad\s?f\.?ly|(ad|u)\s?block|download|ERR_CONNECTION_ABORTED|evassmat|update|infect`,
		unless:       `(?i)nightly|pre[- ]*release|beta|alpha|alfa|((download|get|where).*1[.]15)|multimc`,
	...
	{
		pattern: `(?i)change(\s*logs?|s)`,
		message: "[Changelog](https://impactclient.net/changelog)",
	},
	{
		pattern: `(?i)hack(s|ing|er|client)?`,
	...
	{
		pattern:   `(?i)dumb|retard|idiot`,
		message:   "Like the " + Weeb.Mention() + "s?",
		onlyRoles: []Role{Weeb},
	},
	{
		pattern:      `(?i)premium|donat|become\s*a?\s+don(at)?or|what\*do\s*(you|i|u)\s*(get|unlock)|perks?`,
	...
	{
		pattern:      `(?i)(1\.15.*?(fucking|get|where|need|asap|update|coming|support|release|impact|version|eta|when|out|support)|(fucking|get|where|need|asap|update|coming|support|release|impact|version|eta|when|out|support).*?1\.15)`,
	...
	{
		pattern:   `(?i)nightly|pre[- ]*release|beta|alpha|alfa|((download|get|where).*1[.]15)`,
		message:   "You can install nightly builds of Impact using the **Impact Nightly Installer**: [EXE for Windows](" + strings.Replace(nightlies, "<EXT>", "exe", 1) + ") or [JAR for other platforms](" + strings.Replace(nightlies, "<EXT>", "jar", 1) + ").\nYou can download the normal installer [here](https://impactclient.net/?brady-money-grubbing-completed=true).",
		onlyRoles: []Role{Donator},
	},
	{
	...
	{
		pattern: `(?i)((crack|cracked) (launcher|account|game|minecraft))|(terramining|shiginima|(t(-|)launcher))`,
	...
	{
		pattern: `(?i)(1\.16.*?(update|coming|support|release|impact|version|eta|when|out|support)|(update|coming|support|release|impact|version|eta|when|out|support).*?1\.16)`,
	...
	{
		pattern: `(?i)(impact.+(1\.8|1\.7))|((1\.8|1\.7).impact)`,
