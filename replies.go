...
var replies = []Reply{
	...
	},
	{
		...
		unless:          `forgot|how\s*long|i\s*donated|hours?|wait`,
	...
	{
		pattern:         `forgot\s+.*(name|user|account|discord|mc|minecraft|uuid)|(how\s*long|still\sdon'?t|did|wait).+(premium|donat|become\s*a?\s+don(at)?or)`,
